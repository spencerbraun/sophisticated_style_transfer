...
class Decoder(object):
    ...
    def __init__(self, sess, args, vocab, model):
        ...
        self.inp = tf.compat.v1.placeholder(tf.compat.v1.int32, [None])
        self.h = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, dim_h])
        ...
        with tf.compat.v1.variable_scope('generator'):
            inp = tf.compat.v1.nn.embedding_lookup(embedding, self.inp)
            outputs, self.h_prime = cell(inp, self.h)
            logits = tf.compat.v1.matmul(outputs, proj_W) + proj_b
            log_lh = tf.compat.v1.log(tf.compat.v1.nn.softmax(logits))
            self.log_lh, self.indices = tf.compat.v1.nn.top_k(log_lh, self.beam_width)
