...
class Model(object):
    ...
    def __init__(self, args, vocab):
        ...
        self.learning_rate = tf.compat.v1.placeholder(tf.compat.v1.float32,
            name='learning_rate')
        self.x = tf.compat.v1.placeholder(tf.compat.v1.int32, [None, None],    #batch_size * max_len
            name='x')
        self.y = tf.compat.v1.placeholder(tf.compat.v1.float32, [None],
            name='y')

        embedding = tf.compat.v1.get_variable('embedding', [vocab.size, dim_emb])
        x = tf.compat.v1.nn.embedding_lookup(embedding, self.x)
        self.logits = cnn(x, filter_sizes, n_filters, self.dropout, 'cnn')
        self.probs = tf.compat.v1.sigmoid(self.logits)

        loss = tf.compat.v1.nn.sigmoid_cross_entropy_with_logits(
