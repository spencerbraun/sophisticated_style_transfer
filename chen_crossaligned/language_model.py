...
class Model(object):
    ...
    def __init__(self, args, vocab):
        ...
        self.batch_size = tf.compat.v1.placeholder(tf.compat.v1.int32,
            name='batch_size')
        self.inputs = tf.compat.v1.placeholder(tf.compat.v1.int32, [None, None],    #batch_size * max_len
            name='inputs')
        ...
        inputs = tf.compat.v1.nn.embedding_lookup(embedding, self.inputs)
        cell = create_cell(dim_z, n_layers, self.dropout)
        outputs, _ = tf.compat.v1.nn.dynamic_rnn(cell, inputs,
            dtype=tf.compat.v1.float32, scope='language_model')
        outputs = tf.compat.v1.nn.dropout(outputs, self.dropout)
        outputs = tf.compat.v1.reshape(outputs, [-1, dim_z])
        self.logits = tf.compat.v1.matmul(outputs, proj_W) + proj_b

        loss = tf.compat.v1.nn.sparse_softmax_cross_entropy_with_logits(
            labels=tf.compat.v1.reshape(self.targets, [-1]),
            logits=self.logits)
        loss *= tf.compat.v1.reshape(self.weights, [-1])
        self.tot_loss = tf.compat.v1.reduce_sum(loss)
        self.sent_loss = self.tot_loss / tf.compat.v1.to_float(self.batch_size)
